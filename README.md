...
- Zone - зона доставки (0-10)

**Перевозки** - детали рейсов:
- Дата, ФИО водителя, гос. номер
- Вес груза, расход топлива, прибыль

**ZoneSettings** - настройки стоимости зон:
- ZoneId (0-10), Cost, UpdatedDate
- Используется для расчета стоимости по зонам

**ZoneCostHistory** - история изменений стоимости:
- ZoneId, OldCost, NewCost, ChangeDate, ChangedBy
- Полный аудит всех изменений стоимости зон

**Техобслуживание** - данные техосмотров:
- Номер машины, Дата последнего ТО, Пробег

**OSAGO** - полисы ОСАГО:
- VehicleRegistrationNumber, PolicyNumber, EndDate

**DriverLicenses** - водительские удостоверения:
- DriverFullName, LicenseNumber, ExpiryDate
...
**⚙ Настройки** - управление стоимостью зон и строкой подключения к БД

### Форма настроек

Новая форма настроек включает три вкладки:

1. **Стоимость зон** - редактирование стоимости для зон 0-10
   - Валидация положительных значений
   - Автоматическое сохранение в БД
   - Индикация несохраненных изменений
   
2. **Подключение к БД** - управление строкой подключения
   - Тестирование подключения перед сохранением
   - Валидация формата строки подключения
   - Сохранение в App.config с автоматическим применением
   
3. **История изменений** - лог всех изменений стоимости зон
   - Дата и время изменения
   - Старая и новая стоимость
